...
class Graph:
    ...
    def check_path(self, source, destination):
        ...
        return False
    def min_distance(self,distance_map,visited_places):
        min_distance=sys.maxsize
        min_place=None
        for place,distance in distance_map.items():
            if distance<min_distance and place not in visited_places:
    ...
    def dijkstra(self,source,destination):
        ...
        while destination.name not in visited_places and current is not None:
            ...
            for place,distance in current_place.neighbors.items():
                ...
                if place not in distance_map or new_distance<distance_map[place.name]:
                    distance_map[place.name]=new_distance
            visited_places.add(current)
        if destination.name in visited_places:
            return distance_map[destination.name]
        else:
            return False
